 ...
 */
export async function setupDemo(reportUrl: string, dataUrl?: string): Promise<Demo> {
    /*
     * 리포트에서 사용할 폰트를 등록합니다.
     * {@see https://realreport.co.kr/docs/api/Globals#registerfonts}
     */
    ...
    const [reportForm, dataSet] = await Promise.all([
        ...
        dataUrl ? fetch(dataUrl).then((res) => res.json() as Promise<ReportDataSet>) : undefined,
    ]);

    /*
     * 2. 뷰어 객체 생성과 보고서 양식과 데이터셋 연결
     * {@see https://realreport.co.kr/docs/api/ReportViewer#%EC%83%9D%EC%84%B1%EC%9E%90}
     */
    const viewer = new ReportViewer('viewer', reportForm, dataSet);

    /*
     * 3. 뷰어 미리보기
     * {@see https://realreport.co.kr/docs/api/ReportViewer#preview}
     */
    ...
    /*
     * 각종 뷰어 API 버튼과 연결
     * {@see https://realreport.co.kr/docs/api/ReportViewer#%ED%95%A8%EC%88%98}
     */
